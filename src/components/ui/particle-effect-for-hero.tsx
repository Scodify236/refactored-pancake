...
interface Particle {
  ...
  size: number;
  color: string;
  angle: number; // For organic oscillation
}
...
const PARTICLE_DENSITY = 0.00015; // Particles per pixel squared
...
export const AntiGravityCanvas: React.FC = () => {
  ...
  const initParticles = useCallback((width: number, height: number) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      newParticles.push({
        ...
        color: Math.random() > 0.9 ? '#f0cb87' : '#ffffff', // Custom brand gold accent color
  ...
  const animate = useCallback((time: number) => {
    ...
    gradient.addColorStop(0, `rgba(240, 203, 135, ${pulseOpacity})`); // Custom gold gradient
    ...
    return (
        <nav className="absolute top-0 left-0 w-full z-20 flex justify-between items-center p-6 md:p-8">
            <div className="flex items-center space-x-2">
                 ...
                 <span className="text-white font-medium tracking-wide text-lg">GCX Antigravity</span>
            </div>
            <div className="hidden md:flex space-x-8 text-sm font-medium text-white/70">
                <Link to="/" className="hover:text-white transition-colors">Home</Link>
                <Link to="/proofs" className="hover:text-white transition-colors">Verified Proofs</Link>
                <a href="#brands" className="hover:text-white transition-colors">Rates</a>
            </div>
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center pointer-events-none px-4">
            <div className="max-w-4xl w-full text-center space-y-8">
                ...
                <p className="max-w-2xl mx-auto text-lg md:text-xl text-white/60 font-light leading-relaxed">
                    Redefining card redemption. Interactive zero-gravity mechanics representing transaction flow speed and platform transparency.
                </p>
                ...
                <div className="pt-8 pointer-events-auto">
                    ...
                    >
                        <span className="relative z-10">Start WhatsApp Trade</span>
                        <ArrowRight className="w-4 h-4 relative z-10 group-hover:translate-x-1 transition-transform" />
                        <div className="absolute inset-0 bg-amber-500 transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left duration-300 ease-out opacity-20"></div>
                    </a>
                </div>
            </div>
        </div>
...
export default function ParticleEffectForHero() {
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden selection:bg-amber-500 selection:text-white">
      <AntiGravityCanvas />
      <Navigation />
      <HeroContent />
